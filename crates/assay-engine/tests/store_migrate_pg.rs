...
const SEAL_KEY: &str = "migration-environment-seal-key-with-at-least-thirty-two-characters";
...
async fn populate_sqlite_store(dir: &Path, data_dir: &Path) {
    populate_sqlite_store_with_seal(dir, data_dir, None).await;
}

async fn populate_sqlite_store_with_seal(dir: &Path, data_dir: &Path, seal: Option<&str>) {
    let mut engine = EngineProcess::spawn_with_env(dir, "sqlite", &sqlite_backend(data_dir), seal);
    let http = client();
...
async fn convert_plaintext_kek_to_shamir_metadata(data_dir: &Path) {
    let url = format!("sqlite://{}/vault.db", data_dir.display());
    let pool = sqlx::SqlitePool::connect(&url)
        .await
        .expect("open vault.db");
    let (kid, blob): (String, Vec<u8>) =
        sqlx::query_as("SELECT kid, sealed_blob FROM kek_metadata LIMIT 1")
            .fetch_one(&pool)
            .await
            .expect("read plaintext KEK");
    let key: [u8; 32] = blob.try_into().expect("plaintext KEK is 32 bytes");
    let digest = assay_vault::crypto::kek_store::full_kek_digest(&key);
    sqlx::query(
        "UPDATE kek_metadata
            SET sealing_method = 'shamir', sealed = 1, sealed_blob = x'',
                kek_digest = ?, share_threshold = 3, share_count = 5
    ...
    .execute(&pool)
    .await
    .expect("convert KEK metadata to Shamir");
    pool.close().await;
}
...
#[tokio::test(flavor = "multi_thread")]
async fn migrated_environment_sealed_vault_reopens_with_the_same_method_pg() {
    ...
    let dir = tempfile::tempdir().expect("tempdir");
    let data_dir = dir.path().join("data");
    populate_sqlite_store_with_seal(dir.path(), &data_dir, Some(SEAL_KEY)).await;

    let out = run_migrate(&data_dir, &db.url(), &[]);
    assert!(
        out.status.success(),
        "migrate failed: {}",
        String::from_utf8_lossy(&out.stderr)
    ...
    let mut engine =
        EngineProcess::spawn_with_env(dir.path(), "pg-env", &pg_backend(&db.url()), Some(SEAL_KEY));
    let http = client();
    engine.wait_ready(&http).await.expect("pg engine ready");
    let response = http
        ...
        .await
        .expect("seal status");
    assert_eq!(response.status(), 200, "seal status: {}", engine.log());
    let status: serde_json::Value = response.json().await.expect("seal status json");
    assert_eq!(status["method"], "env-aes-gcm");
    assert_eq!(status["sealed"], false);
}

#[tokio::test(flavor = "multi_thread")]
async fn migrated_shamir_vault_starts_sealed_even_with_an_environment_key_pg() {
    let Some(fx) = fixture().await else {
        eprintln!("skipped: TEST_DATABASE_URL not set");
        return;
    };
    convert_plaintext_kek_to_shamir_metadata(&fx.data_dir).await;
    ...
    let mut engine = EngineProcess::spawn_with_env(
        fx.dir.path(),
        "pg-shamir",
        &pg_backend(&fx.db.url()),
        Some(SEAL_KEY),
    );
    let http = client();
    engine.wait_ready(&http).await.expect("pg engine ready");
    ...
    assert_eq!(response.status(), 200, "seal status: {}", engine.log());
    let status: serde_json::Value = response.json().await.expect("seal status json");
    assert_eq!(status["method"], "shamir");
