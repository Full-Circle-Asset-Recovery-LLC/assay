...
pub async fn run(plan: Plan) -> Result<Report> {
    let source_path = std::path::Path::new(&plan.source_dir);
    let source_lock = crate::process_lock::ProcessLock::acquire(source_path)
        .context("lock SQLite migration source")?;
    let anchored_source = source_lock
        .anchored_data_dir()
        .unwrap_or_else(|| source_path.to_path_buf());
    let anchored_source = anchored_source
        .to_str()
        .ok_or_else(|| anyhow::anyhow!("SQLite migration source is not valid UTF-8"))?;
    let source = crate::init::sqlite_pool(anchored_source).await?;
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn migration_refuses_a_source_owned_by_a_running_engine() {
        ...
        let plan = Plan {
            ...
            target_url: "postgres://127.0.0.1:1/unused".to_string(),
            dry_run: true,
        };

        let err = match run(plan).await {
            Ok(_) => panic!("migration should refuse a source owned by a running engine"),
            Err(err) => err,
        };
        let message = format!("{err:#}");
        assert!(
            message.contains("another assay-engine process holds"),
            "{message}"
        );
        drop(engine_lock);
    }
