...
impl SealKey {
    ...
    pub fn from_env() -> Result<Option<Self>> {
        match std::env::var(ENV_VAR) {
            ...
            Err(std::env::VarError::NotUnicode(_)) => Err(VaultError::Invalid(format!(
...
mod tests {
    ...
    fn non_unicode_environment_value_is_rejected() {
        ...
        const CHILD_MARKER: &str = "ASSAY_TEST_NON_UNICODE_SEAL_KEY_CHILD";
        if std::env::var_os(CHILD_MARKER).is_some() {
            let err = SealKey::from_env().unwrap_err();
            assert!(err.to_string().contains(ENV_VAR), "{err}");
            assert!(err.to_string().contains("valid Unicode"), "{err}");
            return;
        }
        ...
        let output = Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "crypto::env_seal::tests::non_unicode_environment_value_is_rejected",
                "--nocapture",
            ])
            .env(CHILD_MARKER, "1")
            .env(ENV_VAR, OsString::from_vec(vec![0xff, 0xfe]))
            .output()
            .unwrap();
        assert!(
