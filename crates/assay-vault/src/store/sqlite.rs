...
mod items {
    ...
    impl ItemStore for SqliteItemStore {
        async fn create_item(
            ...
            let res = sqlx::query(
                "INSERT INTO vault.items
                    (id, vault_id, collection_id, folder_id, item_type, name, ciphertext, nonce, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            )
            .bind(id)
            .bind(vid)
                ...
                match (vid, cid) {
                    (Some(v), None) => sqlx::query_as(
                        "SELECT id, vault_id, collection_id, folder_id, item_type, name, ciphertext, nonce, created_at, updated_at
                           FROM vault.items WHERE vault_id = ? ORDER BY created_at",
                    )
                    .bind(v)
                    .fetch_all(&self.pool)
                    .await,
                    (None, Some(c)) => sqlx::query_as(
                        "SELECT id, vault_id, collection_id, folder_id, item_type, name, ciphertext, nonce, created_at, updated_at
                           FROM vault.items WHERE collection_id = ? ORDER BY created_at",
                    )
                    .bind(c)
                    .fetch_all(&self.pool)
                    .await,
        ...
        async fn delete_item(&self, id: &str) -> VaultResult<bool> {
            let n = sqlx::query("DELETE FROM vault.items WHERE id = ?")
                .bind(id)
                .execute(&self.pool)
                .await
    ...
    impl FolderStore for SqliteFolderStore {
        async fn create_folder(
            ...
            sqlx::query(
                "INSERT INTO vault.folders
                    (id, vault_id, collection_id, parent_id, name, created_at)
                 VALUES (?, ?, ?, ?, ?, ?)",
            )
            .bind(id)
            .bind(vid)
        ...
        async fn rename_folder(&self, id: &str, name: &str) -> VaultResult<bool> {
            let n = sqlx::query("UPDATE vault.folders SET name = ? WHERE id = ?")
                .bind(name)
                .bind(id)
                .execute(&self.pool)
                .await
        ...
        async fn delete_folder(&self, id: &str) -> VaultResult<bool> {
            let n = sqlx::query("DELETE FROM vault.folders WHERE id = ?")
                .bind(id)
                .execute(&self.pool)
                .await
...
#[cfg(feature = "vault-collections")]
pub use items::{SqliteFolderStore, SqliteItemStore};
...
mod share {
    ...
    impl RevocationStore for SqliteRevocationStore {
        ...
        async fn any_revoked(&self, key_ids: &[String]) -> VaultResult<bool> {
            ...
            // SQLite doesn't have ANY(array) like PG — build a series
            ...
            let placeholders = std::iter::repeat_n("?", key_ids.len())
                ...
                .join(",");
            let q = format!(
                "SELECT 1 FROM vault.share_revoked WHERE key_id IN ({placeholders}) LIMIT 1"
            );
    ...
    pub async fn load_or_init_biscuit_root_sqlite(
        pool: &SqlitePool,
        ...
        sqlx::query(
            "INSERT INTO vault.biscuit_root_keys (kid, private_pem, public_pem, created_at)
             VALUES (?, ?, ?, ?)",
        )
        .bind(&kid)
...
mod dynamic {
    ...
    impl LeaseStore for SqliteLeaseStore {
        async fn create_lease(
            ...
            let md = serde_json::to_string(metadata).unwrap_or_else(|_| "{}".to_string());
            sqlx::query(
                "INSERT INTO vault.leases (id, provider, role, issued_at, expires_at, metadata)
                 VALUES (?, ?, ?, ?, ?, ?)",
            )
            .bind(id)
            .bind(provider)
        ...
        async fn get_lease(&self, id: &str) -> VaultResult<Option<LeaseRecord>> {
            let row: Option<(String, String, f64, f64, Option<f64>, String)> = sqlx::query_as(
                "SELECT provider, role, issued_at, expires_at, revoked_at, metadata
                   FROM vault.leases WHERE id = ?",
            )
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(map_err("get_lease"))?;
            Ok(row.map(|(p, r, ia, ea, ra, md)| LeaseRecord {
                id: id.to_string(),
                provider: p,
                role: r,
                issued_at: ia,
                expires_at: ea,
                revoked_at: ra,
                metadata: serde_json::from_str(&md)
        ...
        async fn revoke_lease(&self, id: &str, now: f64) -> VaultResult<bool> {
            let n = sqlx::query(
                "UPDATE vault.leases SET revoked_at = ?
...
pub use dynamic::SqliteLeaseStore;
