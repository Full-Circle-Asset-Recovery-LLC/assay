...
| `assay.traefik` |  |
| **Security & Identity** | |
| `assay.authz` | In-process authorization engine — policy statements, grants-at-scope with typed bounds, ABAC conditions, deny-wins, asymmetric fail-closed |
| `assay.certmanager` |  |
| `assay.dex` |  |
| `assay.eso` |  |
| `assay.openbao` |  |
...
| `assay.system` |  |
| **Registries** | |
| `assay.bettercontact` | BetterContact waterfall enrichment — submit a batch, poll the run, read verified emails |
| `assay.brreg` | Norway's Enhetsregisteret — keyless company data with websites and live employee counts |
| `assay.companies_house` | UK Companies House — every registered UK company and its officers, by name, number or director |
| `assay.contactout` | ContactOut lead data — LinkedIn enrichment and work-email lookup, budget-gated |
| `assay.cvr` | Denmark's CVR via the public cvrapi.dk gateway — keyless company lookup by name, VAT or phone |
| `assay.edgar` | SEC EDGAR — US public-company registry, filings and full-text search |
| `assay.email_verify` | Email verification — syntax, MX over DoH, list signals, and a live SMTP probe |
| `assay.gleif` | GLEIF LEI registry — global legal-entity search, keyless and free |
| `assay.lead_provider` | The lead-data contract — uniform records with provenance, and the gate every paid call goes through |
| **Stdlib** | |
| `assay.shell` |  |
<!-- END STDLIB TABLE -->