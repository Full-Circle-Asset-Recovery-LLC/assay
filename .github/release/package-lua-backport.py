"""Package a reviewed assay Lua runtime build with immutable source and ABI provenance."""
import hashlib
import json
import os
...
binary = Path("target/release/assay")
version = output(str(binary), "--version")
if version != "assay " + os.environ["EXPECTED_LUA_VERSION"]:
    raise SystemExit("assay version does not match the approved backport: " + version)
...
if not versions or max(versions) > (2, 35):
    raise SystemExit("assay exceeds consumer glibc 2.35 compatibility ceiling")
destination = Path("dist/lua-backport")
destination.mkdir(parents=True, exist_ok=False)
installed = destination / "assay-linux-x86_64"
