...
A present value that is not valid Unicode is also refused. It is never treated as an absent seal
key, so a malformed process environment cannot trigger plaintext KEK bootstrap.
...
## KEK rotation backend support

SQLite KEK rotation drains key-using operations in the engine process, commits the new KEK metadata
and every KV/transit re-wrap in one transaction, and only then exposes the new KEK to requests.

PostgreSQL KEK rotation is currently disabled with an explicit error. A process-local drain cannot
stop another engine instance from writing with its old in-memory KEK after rotation commits. Safe
PostgreSQL rotation requires a distributed writer drain and peer KEK reload protocol; ordinary
PostgreSQL vault reads and writes remain supported.
